...
FILE* OpenBlockFile2(const CDiskBlockPosvvv &pos, bool fReadOnly = false);
...
struct CDiskBlockPosvvv
{
    ...
    CDiskBlockPosvvv(int nFileIn, unsigned int nPosIn) {
        nFile = nFileIn;
        nPos = nPosIn;
    }

    friend bool operator==(const CDiskBlockPosvvv &a, const CDiskBlockPosvvv &b) {
...
struct CDiskTxPosrr : public CDiskBlockPosvvv
{
    unsigned int nTxOffset; // after header

    IMPLEMENT_SERIALIZE(
        READWRITE(*(CDiskBlockPosvvv*)this);
        READWRITE(VARINT(nTxOffset));
    )

    CDiskTxPosrr(const CDiskBlockPosvvv &blockIn, unsigned int nTxOffsetIn) : CDiskBlockPosvvv(blockIn.nFile, blockIn.nPos), nTxOffset(nTxOffsetIn) {
    }
...
public:
    ...
    CBlockIndexversion* pprev;

    // (memory only) pointer to the index of the *active* successor of this block
    CBlockIndexversion* pnext;
    ...
    CBlockIndexversion(CBlock& block)
    {
        phashBlock = NULL;
        pprev = NULL;
        pnext = NULL;
    ...
    int64_t GetMedianTimePast() const
    {
        ...
        const CBlockIndexversion* pindex = this;
        for (int i = 0; i < nMedianTimeSpan && pindex; i++, pindex = pindex->pprev)
            *(--pbegin) = pindex->GetBlockTime();
     ...
     * in the last nToCheck blocks, starting at pstart and going backwards.
     */
    static bool IsSuperMajority(int minVersion, const CBlockIndexversion* pstart,
                                unsigned int nRequired, unsigned int nToCheck);

    std::string ToString() const
    {
        //return strprintf("CBlockIndex(nprev=%p, pnext=%p, nFile=%u, nBlockPos=%-6d nHeight=%d, nMint=%s, nMoneySupply=%s, nFlags=(%s)(%d)(%s), nStakeModifier=%016" PRIx64", nStakeModifierChecksum=%08x, hashProof=%s, prevoutStake=(%s), nStakeTime=%d merkle=%s, hashBlock=%s)",
        return strprintf("CBlockIndexversion(pprev=%p, pnext=%p, nHeight=%d, nMint=%s, nMoneySupply=%s, nFlags=(%s)(%d)(%s), nStakeModifier=%016" PRIx64", nStakeModifierChecksum=%08x, hashProofOfStake=%s, prevoutStake=(%s), nStakeTime=%d merkle=%s, hashBlock=%s)",
            pprev, pnext, nHeight,
            FormatMoney(nMint).c_str(), FormatMoney(nMoneySupply).c_str(),
            GeneratedStakeModifier() ? "MOD" : "-", GetStakeEntropyBit(), IsProofOfStake()? "PoS" : "PoW",
            nStakeModifier, nStakeModifierChecksum, 
...
class CDiskBlockIndexneww : public CBlockIndexversion
{
public:
    uint256 hashPrev;
    ...
    explicit CDiskBlockIndexneww(CBlockIndexversion* pindex) : CBlockIndexversion(*pindex) {
        hashPrev = (pprev ? pprev->GetBlockHash() : 0);
    }
    ...
    (
        ...
        {
            const_cast<CDiskBlockIndexneww*>(this)->prevoutStake.SetNull();
            const_cast<CDiskBlockIndexneww*>(this)->nStakeTime = 0;
            const_cast<CDiskBlockIndexneww*>(this)->hashProofOfStake = 0;
        }
    ...
    std::string ToString() const
    {
        std::string str = "CDiskBlockIndexneww(";
        str += CBlockIndexversion::ToString();
